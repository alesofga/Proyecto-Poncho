...
import uuid
from flask import Flask, request, redirect, render_template_string, session, url_for, flash, jsonify, redirect, url_for, flash
...
def get_db_connection():
    ...
    # Si la columna ya existe saltará OperationalError y lo ignoramos:
    try:
        c.execute("ALTER TABLE Bitacora ADD COLUMN recording_pdf TEXT")
    except sqlite3.OperationalError:
        pass

    # Luego tus CREATE TABLE IF NOT EXISTS…
...
def fetch_and_store_recording(call_id):
    ...
    # Credenciales para la API de Sinch
    ...
    # Carpeta donde guardaremos mp3 y pdf
    base = os.path.join(app.static_folder, "recordings")
    os.makedirs(base, exist_ok=True)

    # Esperamos hasta que la API nos devuelva la URL de la grabación
    recording_url = None
    ...
    mp3_resp = requests.get(recording_url)
    mp3_resp.raise_for_status()
    with open(mp3_path, "wb") as f:
        f.write(mp3_resp.content)

    # Generamos el PDF con un enlace al MP3
    ...
    # Actualizamos la columna recording_pdf en Bitacora
...
def generar_pdf_donantes(nombre_archivo="static/donantes.pdf"):
    ...
    cur.execute("SELECT * FROM Donantes"); donantes = cur.fetchall(); conn.close()

    os.makedirs(os.path.dirname(nombre_archivo), exist_ok=True)
    c = canvas.Canvas(nombre_archivo)
    y = 800
    c.setFont("Helvetica-Bold", 14)
    c.drawString(50, y, "Listado de Donantes")
    ...
    for d in donantes:
        if y < 100:
            c.showPage()
            y = 800
        c.drawString(50, y, f"• {d['nombre_donante']} ({d['tipo_donante']}, {d['sector_donante']})")
        c.drawString(300, y, f"Tel: {d['telefono_donante']}  Correo: {d['correo_donante']}")
        y -= 20
...
def generar_pdf_donante_individual(donante_id, filename):
    ...
    c.setFont("Helvetica-Bold", 14)
    c.drawString(50, 800, f"Donante: {d['nombre_donante']}")
    c.setFont("Helvetica", 12)
    c.drawString(50, 770, f"ID: {d['id']}")
    c.drawString(50, 750, f"Tipo: {d['tipo_donante']}")
    c.drawString(50, 730, f"Sector: {d['sector_donante']}")
    c.drawString(50, 710, f"Teléfono: {d['telefono_donante']}")
    c.drawString(50, 690, f"Correo: {d['correo_donante']}")
    c.drawString(50, 670, f"Dirección: {d['direccion_donante']}")
    c.drawString(50, 650, f"Redes: {d['redes_sociales']}")
    c.drawString(50, 630, "Observaciones:")
    text = c.beginText(50, 610)
    for line in (d['observaciones_donante'] or "").split("\n"):
        text.textLine(line)
    c.drawText(text)
    c.save()
    return filename
...
def notify(reminder_id: int, mensaje: str):
    conn = get_db_connection()
    try:
        conn.execute(
...
def schedule_existing():
    conn = get_db_connection()
    rows = conn.execute(
      "SELECT id, due_datetime, mensaje FROM Reminders WHERE seen=0"
    ).fetchall()
    conn.close()
    for r in rows:
        run_date = datetime.fromisoformat(r["due_datetime"])
        scheduler.add_job(
          notify, 'date', run_date=run_date,
          args=[r["id"], r["mensaje"]],
          id=str(r["id"])
        )
...
def generar_pdf_donantes_route():
    generar_pdf_donantes("static/donantes.pdf")
    return redirect("/static/donantes.pdf")

@app.route("/ver_pdf_donante/<donante_id>")
def ver_pdf_donante(donante_id):
    directory = os.path.join(app.root_path, "static", "pdf_donantes")
    filename = f"{donante_id}.pdf"
    os.makedirs(directory, exist_ok=True)

    filepath = os.path.join(directory, filename)
    generar_pdf_donante_individual(donante_id, filepath)
...
def dashboard():
  ...
  allowfullscreen>
    ...
    # HTML de tabla resumida
    tabla_donantes_html = """
    <h3>Donantes</h3>
    <button class="add-button" onclick="openDonanteForm()">+ Nuevo Donante</button>
    <table class="bitacora-table">
      <thead><tr>
        <th>Nombre</th><th>Sector</th><th>Teléfono</th><th>Acciones</th>
      </tr></thead><tbody>
    """
    for d in donantes:
        ...
        <tr>
          ...
          <td>
            <a href="/ver_pdf_donante/{d['id']}" target="_blank"><i class="fa-solid fa-eye"></i></a>
            <a href="/ver_pdf_donante/{d['id']}" download><i class="fa-solid fa-download"></i></a>
          </td>
        </tr>
        """
...
<div class="app-container">
  <div class="side-nav">
    ...
    <div class="nav-icon" onclick="selectNav(4)"><i class="fa-solid fa-hand-holding-heart"></i></div>
...
<script>
  function openDonanteForm(){
    document.getElementById('donante-form').classList.add('show');
  }
  function closeDonanteForm(){
    document.getElementById('donante-form').classList.remove('show');
  }
</script>
                                  
<!-- Modal Nuevo Donante -->
<div id="donante-form" class="form-container">
  <div class="form-header">
    <h3>Nuevo Donante</h3>
    <button class="close-btn" onclick="closeDonanteForm()">X</button>
  </div>
  <form method="POST" action="{{ url_for('agregar_donante') }}" class="form-body">
    <!-- aquí van todos tus campos -->
    <form method="POST" action="{{ url_for('agregar_donante') }}" class="form-body">
        <div class="form-group">
            <label>Tipo de Donante</label>
            <input type="text" name="tipo_donante" required>
        </div>
        <div class="form-group">
            <label>Sector</label>
            <input type="text" name="sector_donante" required>
        </div>
        <div class="form-group">
            <label>Nombre</label>
        ...
        <div class="form-group">
            ...
            <input type="text" name="telefono_donante">
        </div>
        <div class="form-group">
            <label>Correo</label>
            <input type="email" name="correo_donante">
        </div>
        <div class="form-group">
            <label>Dirección</label>
            <input type="text" name="direccion_donante">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <textarea name="observaciones_donante"></textarea>
        </div>
...
<script>
  ...
  socket.on('nuevo_recordatorio', data => {
    // aquí tu “pop-up”
    alert(data.msg);
    // opcional: envía un fetch para marcarlo seen en BD
  });
</script>
...
""",
        ...
        donantes_html=tabla_donantes_html,   
...
def guardar_contacto():
    ...
    conn.execute("""
        INSERT OR IGNORE INTO Usuario
           (id,nombre,correo,usuario,contraseña,numero_empleado,stars)
        VALUES (?,?,?,?,?,?,?)
    """, (
        data.get("idcontacto"),
    ...
    if data.get("agendar_llamada") == "Si":
        due = data["fecha"] + "T09:00:00"   
        conn.execute(
          "INSERT INTO Reminders (user_id, tipo, mensaje, due_datetime) VALUES (?,?,?,?)",
          (
            session["user_id"],
            "seguimiento",
            f"Llamada de seguimiento a {data['nombre_completo']}",
            due
          )
        )
        conn.commit()

    run_date = datetime.fromisoformat(due)
    scheduler.add_job(
        notify, 'date', run_date=run_date,
        args=[conn.execute("SELECT last_insert_rowid()").fetchone()[0],
          f"Llamada de seguimiento a {data['nombre_completo']}"],
        id=f"reminder_{session['user_id']}_{due}"
    )
    conn.close()
    return redirect(url_for("dashboard"))
...
def llamar(contacto_id):
    ...
    try:
        ...
        # lanza en hilo la descarga de la grabación
        threading.Thread(
...
@app.route('/agregar_donante', methods=['POST'])
def agregar_donante():
    data = request.form
    did = str(uuid.uuid4())
    conn = get_db_connection()
    conn.execute("""
      INSERT INTO Donantes
        (id, tipo_donante, sector_donante, nombre_donante,
